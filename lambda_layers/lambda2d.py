...
class LambdaNetwork2D(tf.keras.layers.Layer):
    def __init__(self,
        ...
        # check parameters
        self._validate_init_args(kernel_out=kernel_out,
                                 heads=heads,
                                 r=receptive_kernel,
                                 size=size,
                                 channel=data_format)
        ...
        self.q_layer = tf.keras.layers.Conv2D(key_depth * heads, 1, use_bias=False, **kwargs)
        self.k_layer = tf.keras.layers.Conv2D(key_depth * intra_depth, 1, use_bias=False, **kwargs)
        self.v_layer = tf.keras.layers.Conv2D(values_depth * intra_depth, 1, use_bias=False, **kwargs)
        ...
        if self.use_context:
            self.embedding = tf.keras.layers.Conv3D(kernel_out, (1, receptive_kernel, receptive_kernel), padding='same',
                                                    use_bias=False)
        else:
            self.embedding = tf.Variable(initial_value=tf.random.uniform(shape=[size, size, key_depth, intra_depth]),
                                         trainable=True,
    ...
    def call(self, inputs: list):
        ...
        if self.use_context:
            ...
            print('v shape rearange ', v.shape)
            lambda_p = self.embedding(v)
            print('lambda before rearange p', lambda_p.shape)
            lambda_p = rearrange(lambda_p, 'b u v hh ww -> b u v (hh ww)')
            print('lambda p', lambda_p.shape)
    ...
    def get_config(self):
        ...
        base_config = super(LambdaNetwork2D, self).get_config()
        ...
        if len(inputs) > 1:
            if inputs[0].shape != inputs[1].shape:
                raise ValueError(
                    ...
                    'and context tensor.'
                    'Given dimension inputs:{}, context: {}'.format(class_name,
                                                                    inputs[0].shape,
                                                                    inputs[1].shape))
    ...
    def _validate_init_args(self, kernel_out, heads, r, size, channel):
        ...
        if (kernel_out % heads) != 0:
            raise ValueError(
                ...
                'Given kernel dimension:{}, heads: {}'.format(class_name,
                                                              kernel_out,
                                                              heads))
        if (r is not None) and (r % 2 != 1):
            raise ValueError(
                '{} layer should have odd receptive kernel size.'
                'Given receptive kernel dimension:{}x{}'.format(class_name,
                                                                r,
                                                                r))
